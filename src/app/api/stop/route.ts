...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await container.stop({ t: 10 }); // stops in 10 seconds
  } catch (error: any) {
    if (error?.statusCode === 304){
      ...
      return NextResponse.json(
        { ok: false, error: "Error stopping container" },
        { status: 500 }
      );
    }
  }
  ...
  return NextResponse.json({
    ok: true,
    message: `Server '${server.name}' (container ${server.containerName}) stopped successfully.`,
  }, { status: 200 });
}
